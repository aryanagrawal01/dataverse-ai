...
def _project_shell(user: UserDTO) -> None:
    ...
    from dataverse.ui.pages_impl import chat, dashboard, data_health, forecast, insights, reports
    ...
    tab_health, tab_dash, tab_insights, tab_chat, tab_forecast, tab_reports = st.tabs(
        ["🩺 Data Health", "📊 Dashboard", "💡 Insights", "💬 Chat", "📈 Forecast", "📄 Reports"]
    )
    with tab_health:
    ...
    with tab_forecast:
        forecast.render(user, project_id)
    with tab_reports:
        reports.render(user, project_id)
